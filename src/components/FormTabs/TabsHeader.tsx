 ...
 */
const TabsHeader = ({ names, currentindex }: TabsHeaderProps) => {
  return (
    <div className="mb-4 overflow-x-auto whitespace-nowrap pb-1 text-center lg:flex lg:items-center lg:justify-around">
      {names.map((name, i) => {
        if (names.length - 1 === i) {
          return <TabHeaderMarker key={i} index={i} name={name} currentindex={currentindex} />;
        }
        return (
          <Fragment key={i}>
            <TabHeaderMarker name={name} index={i} currentindex={currentindex} />
            <span
...
interface TabsHeaderMarkerProps {
  index: number;
  name: string;
  currentindex: number;
}

const TabHeaderMarker = ({ index, name, currentindex }: TabsHeaderMarkerProps) => {
  const ref = useRef<HTMLSpanElement>(null);

  useEffect(() => {
    if (currentindex === index && ref.current !== null) {
  ...
  return (
    ...
    >
      <span className="inline-block border-r border-accent px-2 py-1">{index + 1}.</span>
